...
if ENV:
    ...
    try:
        FROG_USERS = set(
            int(x) for x in os.environ.get("FROG_USERS", "").split())
    except ValueError:
        raise Exception(
            "Your frog users list does not contain valid integers.")
...
telethn = TelegramClient("megu", API_ID, API_HASH)
pyrogrm = Client("MeguPyro", api_id=API_ID,
              api_hash=API_HASH,
              bot_token=TOKEN)
...
from MeguBot.modules.helper_funcs.handlers import (CustomCommandHandler,
