...
def get_user_list(config, key):
    with open('{}/MeguBot/{}'.format(os.getcwd(), config),
              'r') as json_file:
...
class Config(object):
    ...
    JOIN_LOGGER = -1001335261915  #Prints any new group the bot is added to, prints just the name and ID.
    GBAN_LOGS = -1001191829350  #Prints information like gbans, sudo promotes, AI enabled disable states that may help in debugging and shit
    ...
    CASH_API_KEY = 'API'  # Get your API key from https://www.alphavantage.co/support/#api-key
    TIME_API_KEY = 'API'  # Get your API key from https://timezonedb.com/api
    WALL_API = 'API'  #For wallpapers, get one from https://wall.alphacoders.com/api.php
    AI_API_KEY = 'API'  #For chatbot, get one from https://coffeehouse.intellivoid.net/dashboard
