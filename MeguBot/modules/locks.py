...
import MeguBot.modules.sql.locks_sql as sql
from MeguBot import dispatcher, SUDO_USERS, LOGGER
...
def locktypes(update, context):
    update.effective_message.reply_text(
        "\n • ".join(["Tipos de bloqueos disponibles: "] +
    ...
    if (can_delete(chat, context.bot.id) or
        ...
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                send_message(
                    update.effective_message,
                    "¿Qué estás intentando bloquear?...\nPruebe `/locktypes` para la lista de bloqueables", parse_mode="markdown"
                )
        else:
    ...
    if is_user_admin(chat, message.from_user.id):
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                send_message(
                    update.effective_message,
                    "Qué estás intentando desbloquear?... \nPruebe `/locktypes` para ver la lista de bloqueables.", parse_mode="markdown"
                )
