from MeguBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from MeguBot import SUDO_USERS
...
async def megu_is_admin(chat_id: int):
    status = False
    megu = await telethn.get_me()
    async for user in telethn.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if megu.id == user.id:
            status = True
