...
from MeguBot.modules.sql import BASE, SESSION
...
class FedsUserSettings(BASE):
    ...
    def __repr__(self):
        return "<Configuración de reportes de federaciones ({})>".format(self.user_id)
...
class FedSubs(BASE):
    ...
    def __repr__(self):
        return "<Federación {} se suscribe a {}>".format(self.fed_id, self.fed_subs)
...
def new_fed(owner_id, fed_name, fed_id):
    with FEDS_LOCK:
        ...
        fed = Federations(
            ...
            'Las reglas no están establecidas en esta federación.', None,
            str({
                'owner': str(owner_id),
                'members': '[]'
            }))
        ...
        FEDERATION_BYOWNER[str(owner_id)] = ({
            'fid': str(fed_id),
            'fname': fed_name,
            'frules': 'Las reglas no están establecidas en esta federación.',
            'flog': None,
            'fusers': str({
                'owner': str(owner_id),
                'members': '[]'
