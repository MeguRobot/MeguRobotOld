...
from MeguBot.modules.helper_funcs.handlers import (CMD_STARTERS,
                                                        SpamChecker)
...
if is_module_loaded(FILENAME):
    ...
    from MeguBot.modules.sql import disable_sql as sql
        ...
        if len(args) >= 1:
            ...
            if disable_cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                ...
                update.effective_message.reply_text(
                    f"Se desabilitó el uso de `{disable_cmd}`",
                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text(
                    "Ese comando no se puede desabilitar")
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "MeguBot.modules." + args[0].rsplit(".", 1)[0]
            ...
            try:
                ...
                update.effective_message.reply_text(
                    "Este módulo no tiene lista de comandos!")
                return
            ...
            if disabled_cmds:
                ...
                update.effective_message.reply_text(
                    f"Se desabilitó el uso de `{disabled_cmds_string}`",
                    parse_mode=ParseMode.MARKDOWN)

            if failed_disabled_cmds:
                failed_disabled_cmds_string = ", ".join(failed_disabled_cmds)
                update.effective_message.reply_text(
                    f"No se pudo desabilitar el uso de `{failed_disabled_cmds_string}`",
                    parse_mode=ParseMode.MARKDOWN)
    ...
    def enable(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                update.effective_message.reply_text(
                    f"Se habilitó el uso de `{enable_cmd}`",
                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("Estaba desabilitado?")
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            enable_module = "MeguBot.modules." + args[0].rsplit(".", 1)[0]
            ...
            if enabled_cmds:
                ...
                update.effective_message.reply_text(
                    f"Se habilitó el uso de `{enabled_cmds_string}`",
                    parse_mode=ParseMode.MARKDOWN)
    ...
    def list_cmds(update: Update, context: CallbackContext):
        if DISABLE_CMDS + DISABLE_OTHER:
            ...
            update.effective_message.reply_text(
                f"Comandos que se pueden desabilitar:\n{result}",
                parse_mode=ParseMode.MARKDOWN)
        else:
            update.effective_message.reply_text("No hay comandos para desabilitar.")

    # do not async
    def build_curr_disabled(chat_id: Union[str, int]) -> str:
        disabled = sql.get_all_disabled(chat_id)
        if not disabled:
            return "No hay comandos desabilitados!"
    ...
    def __stats__():
        return f"{sql.num_disabled()} comandos desabilitados, en {sql.num_chats()} chats."
